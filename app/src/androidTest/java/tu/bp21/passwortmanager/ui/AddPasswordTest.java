...
public class AddPasswordTest {
    ...
    void setUp() throws Exception {
        if (mainActivity == null) {
            ...
            randomEmail = generateRandomString(stringLength) + "@" + generateRandomString(smallStringLength) + "." + generateRandomString(smallStringLength);
            do {
            ...
            onWebView().withElement(findElement(Locator.ID, "signup-password")).perform(webKeys(randomMasterPassword));
            onWebView().withElement(findElement(Locator.ID, "signup-password-confirm")).perform(webKeys(randomMasterPassword));
        ...
        //login
        onWebView().withElement(findElement(Locator.ID, "input-username")).perform(webKeys(randomUsername));
        onWebView().withElement(findElement(Locator.ID, "input-userPassword")).perform(webKeys(randomMasterPassword));
    ...
    void addPasswordSuccess() throws Exception{
        ...
        Thread.sleep(3000);
        onWebView().withElement(findElement(Locator.CSS_SELECTOR, "input[placeholder=Webseite]")).perform(webKeys(randomWebsiteName));
        onWebView().withElement(findElement(Locator.CSS_SELECTOR, "input[placeholder=Loginname]")).perform(webKeys(randomLoginName));
        onWebView().withElement(findElement(Locator.CSS_SELECTOR, "input[placeholder=Passwort]")).perform(webKeys(randomLoginPassword));
